...
class AutismCreature(Entity):
    def __init__(self,**kwargs):
        super().__init__(parent=sus, model='quad',texture='assets/textures/saddydaddy.png', scale_y=3,scale_x=3, collider='box',y=2,double_sided=True, **kwargs)
        global goforward
        goforward=True
    def update(self):
        dist = distance_2d(Harlod.position, self.position)
        if dist>1.2 and dist<18:
            self.look_at_2d(Harlod.position, 'y')
            self.position += self.forward * time.dt * 10
            if not yippechase.playing:
                yippechase.play()
        elif dist<1.2:
            if not jumpscare.playing:
                global nomove
                jumpscare.play()
                ObungaDeath=Entity(model='quad',texture='assets/textures/saddydaddy',parent=camera.ui,scale_x=2)
                nomove=True
                Harlod.y=70
                obungaafterscarexpos=Harlod.x - 20
                self.x=obungaafterscarexpos
...
def game_begin():
    global load_bg,start,health_bar_2,health_bar_1,ObungaEnabled
    if ObungaEnabled.value==True:
        global obunga

...
def bb():
    if gamebegun:
        destroy(ee)
        global health_bar_1
        health_bar_1 = HealthBar(bar_color=color.yellow.tint(-.25), roundness=.5, value=50)
    ...
    if gamebegun:
        ...
        class Obunga(Entity):
            ...
            def __init__(self, **kwargs):
                super().__init__(parent=shootables_parent, model='cube',texture='assets/textures/obunga', scale_y=3,scale_x=3, origin_y=-.55,color=color.light_gray, collider='box',y=Harlod.y, **kwargs,)
            ...
            def update(self):
                ...
                hit_info = raycast(self.world_position + Vec3(0,0,0), self.forward, 30, ignore=(self,))
                if hit_info.entity == Harlod:
        ...
        class Andrew(Entity):
            ...
            chase=False
            def __init__(self, **kwargs):
                super().__init__(parent=shootables_parent, model='cube',texture='assets/textures/andrew', scale_y=3,scale_x=3, origin_y=-17.75, color=color.light_gray, collider='box', **kwargs)
            ...
            def update(self):
                ...
                if hit_info.entity == Harlod:
                    if dist > 2:
                        self.position += self.forward * time.dt * 5.5
                        chase=True
        ...
        for i in range(64):
            Entity(model='cube', origin_y=-.2, scale=2, texture='brick', texture_scale=(1,2),
                x=random.uniform(-8,20),
                z=random.uniform(-8,20) + 8,
                collider='box',

...
actor1=Actor("John.gltf")
actor1.setScale(1.3)
actor.setColor(0.128, 0.128, 0.128)
actor1.setColor(0.128, 0.128, 0.128)
actor.setScale(1.3)
actor.play("Armature|test|Anima_Layer")
actor.stop()
ground = Entity(model='plane', collider='box', scale=64, texture='grass', texture_scale=(4,4))
class speaker(Entity):
    def __init__(self, **kwargs):
        super().__init__(parent=hits, model='cube',texture='white_cube', scale=1.2, origin_y=-0.5, collider='box',y=1,alpha=0, **kwargs)   
    def input(self,key):
...
class Johnny(Entity):
    def __init__(self, **kwargs):
        super().__init__(parent=hits, model='cube',texture='white_cube', scale_y=1,scale_x=1, origin_y=-0.5, collider='box',alpha=0,y=1, **kwargs)   
    def input(self,key):
        dist = distance_xz(player.position, self.position)
        if dist > 10:
            return dist
        if dist<3:
            if self.hovered:
                if key=='e':
                    actor1.play("Armature|John|Anima_Layer")
                    player.ignore=True
                    mouse.locked=False
                    global metjohn1,metjohn
                    metjohn=True
...
Character=speaker(x=3*4)
John=Johnny(z=2*4)
actor.reparentTo(scene)
xspeak=Character.x
yspeak=0
zspeak=Character.z
actor1.reparentTo(scene)
x2speak=John.x
y2speak=0
z2speak=John.z
"actor1.setHpr(90,0,0)"
actor1.setPos(x2speak, y2speak, z2speak)
print(xspeak,yspeak,zspeak)
actor.setHpr(-90,0,0)
actor.setPos(xspeak, yspeak, zspeak)
app.run()
...
def level2():
    global level2load,loadnextlevel
    if level2load:
        level2load=False
...
def update():
    global summonFalseEscape,summonedkey1,endlevel1,DoorOneOpened,loadnextlevel,levelpos,level2load,playyourcell2,youareincell1,showedheychuckalready,heychuck
    ...
    if playyourcell2:
        playyourcell2=False
        yourcell2.play()
        def summonFalseEscapeFunc():
            FalseEscapeSummon=[FalseEscape(x=62) in range(1)]
        invoke(summonFalseEscapeFunc,delay=15)
    if youareincell1:
        youareincell1=False
        yourcell3.play()
...
#This is getting to confusing for me so I'll give it a shot later gotta figure out what to do for now